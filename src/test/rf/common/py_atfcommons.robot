...
gal100
    ...
    run keyword and expect error  *received unknown Object type*  Get As List   ${float}
...
cifexist4
    ${a}=     Check If Exists  ${var_empty}  ignore_empty=True
    Should Be Equal As Strings  ${a}  True
    ${a}   ${b}=     Check If Exists and return type  ${var_empty}    ignore_empty=True
    Should Be Equal As Strings  ${a}  True
...
cifexists100
    ${float}=   Set Variable  ${3.14}
    run keyword and expect error  *received unknown Object type*  Check If Exists and return type   ${float}
    run keyword and expect error  *received unknown Object type*  Check If Exists                   ${float}
...
#       ######   #######  ######    #####    #####        #######  #######   #####
dbglog000a
    run keyword and expect error   *unknown mode*  Debug Log    Foolog      debug_log=True  mode=foo1

dbglog000b
    Set Test Variable   ${DBG_LOG}     True
    run keyword and expect error   *unknown mode*  Debug Log    Foolog                      mode=foo1
